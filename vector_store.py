...
class VectorStoreManager:
    ...
    def clear_working_memory(self):
        """Clear all contents from working memory directory and refresh the store."""
        import shutil
        
        # Remove all contents from working directory
        if self.working_path.exists():
            for item in self.working_path.iterdir():
                if item.is_dir():
                    ...
                    item.unlink()
            print(f"Cleared all contents from {self.working_path}")
        
        # Remove the working index
